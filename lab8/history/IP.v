...
GATED_OR GATED_input (
	.CLOCK(clk),
	.SLEEP_CTRL(G_sleep_input),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(G_clock_input)
);

wire G_clock_shar;
wire G_sleep_shar = ctrl_shar&cg_en;
GATED_OR GATED_shar (
	.CLOCK(clk),
...
wire G_clock_mean;
wire G_sleep_mean = ctrl_mean&cg_en;
GATED_OR GATED_mean (
	.CLOCK(clk),
...
wire G_clock_midp;
wire G_sleep_midp = ctrl_midp&cg_en;
GATED_OR GATED_midp (
	.CLOCK(clk),
...
always @(*) begin
	ctrl_input = !(next_state==S_INIMG|curr_state==S_INIMG|curr_state==S_IDLE);
	ctrl_mean = !(curr_state==S_INCMD|curr_state==S_PAD|curr_state==S_PROC);
	ctrl_midp = !(curr_state==S_INCMD|curr_state==S_PAD|curr_state==S_PROC);
	ctrl_shar = !(curr_state==S_INCMD|curr_state==S_PAD|curr_state==S_PROC);
