...
generate
	for(cnt_idx=0;cnt_idx<8;cnt_idx=cnt_idx+1)begin:gated_input_array
		GATED_OR GATED_input__(.CLOCK(clk), .SLEEP_CTRL(cg_en&(~(next_state==S_INPUT_1&cnt==cnt_idx+1))), .RST_N(rst_n),	.CLOCK_GATED(G_clock_input_[cnt_idx]));
	end
endgenerate
...
generate
	for(cnt_idx2=0;cnt_idx2<15;cnt_idx2=cnt_idx2+1)begin:gated_img_array
		GATED_OR GATED_input__(.CLOCK(clk), .SLEEP_CTRL(cg_en&(~((next_state==S_INPUT_1|next_state==S_INPUT_2)&(cnt>13-cnt_idx2)))), .RST_N(rst_n),	.CLOCK_GATED(G_clock_img_[cnt_idx2]));
	end
endgenerate
...
always @(*) begin
	...
	ctrl_input = !((current_state==S_IDLE|next_state==S_INPUT_1)&cnt<'d10);
	...
	ctrl_maxmap0 = !((next_state==S_INPUT_1|current_state==S_INPUT_2)&(cnt<'d2|(cnt=='d25)));
	//ctrl_maxmap1 = 'b1;
	...
	ctrl_nn2b = !(next_state==S_CNN_2|current_state==S_CNN_2);
end
