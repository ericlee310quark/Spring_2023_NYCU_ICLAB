...
reg [inst_sig_width+inst_exp_width:0] h_result [8:0];
...
//reg [inst_sig_width+inst_exp_width:0] mac1_d, mac2_d, mac3_d, mac4_d, mac5_d;
...
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance) A1 (.a(mac1_c), .b(m1_z), .rnd(3'b000), .z(mac1_z));
DW_fp_mult #(inst_sig_width, inst_exp_width, inst_ieee_compliance) M2 (.a(mac2_a), .b(mac2_b), .rnd(3'b000), .z(m2_z));
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance) A2 (.a(mac2_c), .b(m2_z), .rnd(3'b000), .z(mac2_z));
DW_fp_mult #(inst_sig_width, inst_exp_width, inst_ieee_compliance) M3 (.a(mac3_a), .b(mac3_b), .rnd(3'b000), .z(m3_z));
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance) A3 (.a(mac3_c), .b(m3_z), .rnd(3'b000), .z(mac3_z));
DW_fp_mult #(inst_sig_width, inst_exp_width, inst_ieee_compliance) M4 (.a(mac4_a), .b(mac4_b), .rnd(3'b000), .z(m4_z));
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance) A4 (.a(mac4_c), .b(m4_z), .rnd(3'b000), .z(mac4_z));
DW_fp_mult #(inst_sig_width, inst_exp_width, inst_ieee_compliance) M5 (.a(mac5_a), .b(mac5_b), .rnd(3'b000), .z(m5_z));
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance) A5 (.a(mac5_c), .b(m5_z), .rnd(3'b000), .z(mac5_z));
/*
...
//		OUTPUT CTR
//================================
assign output_data = (cnt>'d21)?rec0_z:ans[0];


always@(posedge clk or  negedge rst_n)begin
	if(~rst_n)	out_valid <= 'b0;
	else if(cnt>'d24)	out_valid <= 'b0;
	else if(cnt>'d15)	out_valid <= 'b1;
end
always@(posedge clk or  negedge rst_n)begin
	if(~rst_n)	out <= 'b0;
	else if(cnt>'d24)	out <= 'b0;
	else if(cnt>'d15)	out <= output_data;
end
//=================================
//		FSM CTR
//================================
...
always@(*)begin
	...
	INPUT: 	if(cnt=='d25)		next_stage =IDLE;
...
always@(*)begin
	...
	5'd8:begin
		mac1_a = w_u[7];	mac1_b = d_x[1];				mac1_c = x_mac_result_L20;
		mac2_a = w_w[7];	mac2_b = d_h[1];				mac2_c = h_mac_result_L20;
	...
	5'd9:begin
		mac1_a = w_u[8];	mac1_b = d_x[2];				mac1_c = x_mac_result_L20;
		mac2_a = w_w[8];	mac2_b = d_h[2];				mac2_c = h_mac_result_L20;
		mac3_a = w_w[4];	mac3_b = h_result[1]; 			mac3_c = h_mac_result_L11;
		mac4_a = w_u[8];	mac4_b = d_x[5]; 				mac4_c = x_mac_result_L21;
		mac5_a = w_u[1];	mac5_b = d_x[7]; 				mac5_c = x_mac_result_L00;//x_mac_result_L02;
	...
	5'd10:begin
		mac1_a = w_u[2];	mac1_b = d_x[8];				mac1_c = x_mac_result_L00;//x_mac_result_L02;
		...
		m0_a   = a0_z;		m0_b   = POINT_ONE;
		a0_a   = x_mac_result_L20;		a0_b   = h_mac_result_L20;
	...
	5'd13:begin
		mac1_a = w_v[0];	mac1_b = h_result[3];			mac1_c = 'b0;
		mac2_a = w_v[8];	mac2_b = h_result[2];			mac2_c = v_mac_result_L20;
		mac4_a = w_w[0];	mac4_b = h_result[3];			mac4_c = 'b0;
		...
		mac3_a = exp_reg;   mac3_b = ONE;                   mac3_c = ONE;//a1_a   = exp_reg;		a1_b   = ONE;
	...
	5'd14:begin
		mac1_a = w_v[1];	mac1_b = h_result[4];			mac1_c = v_mac_result_L01;
		mac2_a = w_v[3];	mac2_b = h_result[3];			mac2_c = 'b0;
		mac4_a = w_w[1];	mac4_b = h_result[4];			mac4_c = h_mac_result_L02;
		mac5_a = w_w[3];	mac5_b = h_result[3];	 		mac5_c = 'b0;
		m0_a   = a0_z;		m0_b   = POINT_ONE;
		...
		mac3_a = exp_reg;   mac3_b = ONE;                   mac3_c = ONE;
	...
	5'd15:begin
		mac1_a = w_v[2];	mac1_b = h_result[5];			mac1_c = v_mac_result_L01;
		mac2_a = w_v[4];	mac2_b = h_result[4];			mac2_c = v_mac_result_L11;
		mac3_a = w_v[6];	mac3_b = h_result[3]; 			mac3_c = 'b0;
		mac4_a = w_w[2];	mac4_b = h_result[5];			mac4_c = h_mac_result_L02;
		mac5_a = w_w[4];	mac5_b = h_result[4];	 		mac5_c = h_mac_result_L12;
		m0_a   = w_w[6];	m0_b   = h_result[3];
		a0_a   = exp_reg;		a0_b   = ONE;
        //a1_a   = exp_reg;		a1_b   = ONE;
		rec0_a = add_exp_reg;
	...
	5'd16:begin
		mac2_a = w_v[5];	mac2_b = h_result[5];			mac2_c = v_mac_result_L11;
		mac3_a = w_v[7];	mac3_b = h_result[4]; 			mac3_c = v_mac_result_L21;
		mac4_a = w_w[5];	mac4_b = h_result[5];			mac4_c = h_mac_result_L12;
		mac5_a = w_w[7];	mac5_b = h_result[4];	 		mac5_c = h_mac_result_L22;
		m0_a   = a0_z;		m0_b   = POINT_ONE;
		a0_a   =x_mac_result_L00 ;		a0_b   = h_mac_result_L02;	//x_mac_result_L02
		exp0_a  = v_mac_result_L01;//{(~v_mac_result_L01[31]),v_mac_result_L01[30:0]};//v_mac_result_L01;
		rec0_a = add_exp_reg;
	...
	5'd17:begin
		mac1_a = w_v[0];	mac1_b = h_result[6];			mac1_c = 'b0;
		mac3_a = w_v[8];	mac3_b = h_result[5]; 			mac3_c = v_mac_result_L21;
		mac5_a = w_w[8];	mac5_b = h_result[5];	 		mac5_c = h_mac_result_L22;
		m0_a   = a0_z;		m0_b   = POINT_ONE;
		a0_a   = x_mac_result_L12;		a0_b   = h_mac_result_L12;
		exp0_a = v_mac_result_L11;//{(~v_mac_result_L11[31]),v_mac_result_L11[30:0]};//v_mac_result_L11;
		mac2_a = exp_reg;   mac2_b = ONE;                   mac2_c = ONE;
        //a1_a   = exp_reg;		a1_b   = ONE;
	...
	5'd18:begin
		mac1_a = w_v[1];	mac1_b = h_result[7];			mac1_c = v_mac_result_L02;
		mac2_a = w_v[3];	mac2_b = h_result[6];			mac2_c = 'b0;
		m0_a   = a0_z;		m0_b   = POINT_ONE;
		a0_a   = x_mac_result_L22;		a0_b   = h_mac_result_L22;
		exp0_a  = v_mac_result_L21;//{(~v_mac_result_L21[31]),v_mac_result_L21[30:0]};//v_mac_result_L21;
		mac3_a = exp_reg;   mac3_b = ONE;                   mac3_c = ONE;
        //a1_a   = exp_reg;		a1_b   = ONE;
		rec0_a = add_exp_reg;
	...
	5'd19:begin
		mac1_a = w_v[2];	mac1_b = h_result[8];			mac1_c = v_mac_result_L02;
		mac2_a = w_v[4];	mac2_b = h_result[7];			mac2_c = v_mac_result_L12;
		mac3_a = w_v[6];	mac3_b = h_result[6]; 		mac3_c = 'b0;
		a0_a   = exp_reg;		a0_b   = ONE;
        //a1_a   = exp_reg;		a1_b   = ONE;
		rec0_a = add_exp_reg;
	...
	5'd20:begin
		mac2_a = w_v[5];	mac2_b = h_result[8];			mac2_c = v_mac_result_L12;
		mac3_a = w_v[7];	mac3_b = h_result[7];			mac3_c = v_mac_result_L22;
		exp0_a  = v_mac_result_L02;//{(~v_mac_result_L02[31]),v_mac_result_L02[30:0]};//v_mac_result_L02;
		rec0_a = add_exp_reg;
	...
	5'd21:begin
		mac3_a = w_v[8];	mac3_b = h_result[8];			mac3_c = v_mac_result_L22;
		exp0_a  = v_mac_result_L12;//{(~v_mac_result_L12[31]),v_mac_result_L12[30:0]};//v_mac_result_L12;
		a0_a   = exp_reg;		a0_b   = ONE;
        //a1_a   = exp_reg;		a1_b   = ONE;
		rec0_a = add_exp_reg;
	...
	5'd22:begin
		exp0_a  = v_mac_result_L22;//{(~v_mac_result_L22[31]),v_mac_result_L22[30:0]};//v_mac_result_L22;
		a0_a   = exp_reg;		a0_b   = ONE;
		//a1_a   = exp_reg;		a1_b   = ONE;
		rec0_a = add_exp_reg;
	...
	5'd25, 5'd26, 5'd27, 5'd28, 5'd29, 5'd30, 5'd31:begin
		mac1_a = 'b0;	mac1_b = 'b0;	mac1_c = 'b0;
		mac2_a = 'b0;	mac2_b = 'b0;	mac2_c = 'b0;
...
always@(posedge clk)begin
	if(cnt=='d0)begin
		...
		x_mac_result_L20 <= 'b0; x_mac_result_L21 <= 'b0; x_mac_result_L22 <= 'b0;
		
		h_mac_result_L00 <= 'b0; h_mac_result_L01 <= 'b0; h_mac_result_L02 <= 'b0;
		h_mac_result_L10 <= 'b0; h_mac_result_L11 <= 'b0; h_mac_result_L12 <= 'b0;
		h_mac_result_L20 <= 'b0; h_mac_result_L21 <= 'b0; h_mac_result_L22 <= 'b0;
		...
		5'd7:begin
			x_mac_result_L20 <= mac1_z;
			h_mac_result_L20 <= mac2_z;
			h_mac_result_L21 <= mac3_z;
			x_mac_result_L21 <= mac4_z;
			x_mac_result_L00 <= mac5_z;	//x_mac_result_L02
		...
		5'd10:begin
			x_mac_result_L00 <= mac1_z; //x_mac_result_L02
		...
		5'd11:begin
			...
			v_mac_result_L00 <= {(~mac2_z[31]), mac2_z[30:0]};//mac2_z;
		...
		5'd12:begin
			...
			if(a0_z[31])	h_result[3] <= m0_z;
			else			h_result[3] <= a0_z;
			exp_reg <= exp0_z;
		end
		5'd13:begin
			v_mac_result_L01 <= mac1_z;
			v_mac_result_L20 <= {(~mac2_z[31]), mac2_z[30:0]};//mac2_z;
			h_mac_result_L02 <= mac4_z;
			h_mac_result_L21 <= mac5_z;
			if(a0_z[31])	h_result[4] <= m0_z;
			else			h_result[4] <= a0_z;
			exp_reg <= exp0_z;
			add_exp_reg <= mac3_z;//add_exp_reg <= a1_z;

		end
		5'd14:begin			////////////////////
			v_mac_result_L01 <= mac1_z;
			v_mac_result_L11 <= mac2_z;
			h_mac_result_L02 <= mac4_z;
			h_mac_result_L12 <= mac5_z;
			if(a0_z[31])	h_result[5] <= m0_z;
			else			h_result[5] <= a0_z;
			exp_reg <= exp0_z;
			add_exp_reg <= mac3_z;//add_exp_reg <= a1_z;
			ans[0] <= rec0_z;
		end
		5'd15:begin			////////////////////
			v_mac_result_L01 <= {(~mac1_z[31]), mac1_z[30:0]};//mac1_z;
			v_mac_result_L11 <= mac2_z;
			v_mac_result_L21 <= mac3_z;
			h_mac_result_L02 <= mac4_z;
			h_mac_result_L12 <= mac5_z;
			h_mac_result_L22 <= m0_z;
			add_exp_reg <= a0_z;//add_exp_reg <= a1_z;
			ans[1] <= rec0_z;
		
		end
		5'd16:begin			////////////////////
			v_mac_result_L11 <= {(~mac2_z[31]), mac2_z[30:0]};//mac2_z;
			v_mac_result_L21 <= mac3_z;
			h_mac_result_L12 <= mac4_z;
			h_mac_result_L22 <= mac5_z;
			if(a0_z[31])	h_result[6] <= m0_z;
			else			h_result[6] <= a0_z;			
			exp_reg <= exp0_z;
			ans[1] <= rec0_z;
			ans[0] <= ans[1];
		end
		5'd17:begin			////////////////////
			v_mac_result_L02 <= mac1_z;
			v_mac_result_L21 <= {(~mac3_z[31]), mac3_z[30:0]};//mac3_z;
			h_mac_result_L22 <= mac5_z;
			if(a0_z[31])	h_result[7] <= m0_z;
			else			h_result[7] <= a0_z;			
			exp_reg <= exp0_z;
			add_exp_reg <= mac2_z;//add_exp_reg <= a1_z;
			ans[0] <= ans[1];
		end
		5'd18:begin			////////////////////
			v_mac_result_L02 <= mac1_z;
			v_mac_result_L12 <= mac2_z;
			if(a0_z[31])	h_result[8] <= m0_z;
			else			h_result[8] <= a0_z;			
			exp_reg <= exp0_z;
			add_exp_reg <= mac3_z;//add_exp_reg <= a1_z;
			//ans[3] <= rec0_z;
		...
		5'd19:begin			////////////////////
			v_mac_result_L02 <= {(~mac1_z[31]), mac1_z[30:0]};//mac1_z;
			v_mac_result_L12 <= mac2_z;
			v_mac_result_L22 <= mac3_z;
			add_exp_reg <= a0_z;
            //add_exp_reg <= a1_z;
			//ans[4] <= rec0_z;
			ans[0] <= rec0_z;
		...
		5'd20:begin			////////////////////
			v_mac_result_L12 <= {(~mac2_z[31]), mac2_z[30:0]};//mac2_z;
			v_mac_result_L22 <= mac3_z;
			exp_reg <= exp0_z;
			//ans[5] <= rec0_z;
			ans[0] <= rec0_z;
		...
		5'd21:begin			////////////////////
			v_mac_result_L22 <= {(~mac3_z[31]), mac3_z[30:0]};//mac3_z;
			exp_reg <= exp0_z;
            add_exp_reg <= a0_z;
			//add_exp_reg <= a1_z;
		end
		...
		5'd24, 5'd25, 5'd26, 5'd27, 5'd28, 5'd29, 5'd30, 5'd31:begin			////////////////////		
			...
			x_mac_result_L20 <= x_mac_result_L20; x_mac_result_L21 <= x_mac_result_L21; x_mac_result_L22 <= x_mac_result_L22;
			...
			h_mac_result_L20 <= h_mac_result_L20; h_mac_result_L21 <= h_mac_result_L21; h_mac_result_L22 <= h_mac_result_L22;
